...
def obtener_recorrido(Grafo, source_node="A", tipo_busqueda="DFS"):
...
def setup_graph(ax):
    ...
    Grafo.add_edges_from([("A", "B"), ("A", "C"), ("A", "D"), ("C", "D")])
    ...
    ax.clear()
    nx.draw_networkx_edges(Grafo, pos, ax=ax, width=1.2)
    nx.draw_networkx_nodes(Grafo, pos, node_size=2000, node_color='lightgray', ax=ax)
    nx.draw_networkx_labels(Grafo, pos, font_size=20, font_weight='bold', ax=ax)
    ax.set_axis_off()

    luz = ax.scatter([], [], s=2500, c='yellow', edgecolors='black', linewidths=1.2, zorder=3)
...
def ejecutar_tipo(tipo):
    ...
    recorrido = obtener_recorrido(Grafo, source_node="A", tipo_busqueda=tipo)
...
def ejecutar_manual(event):
    ...
    nodos = [n.strip().upper() for n in entrada.split(',') if n.strip().upper() in pos_base]
    if not nodos:
...
textbox_manual = TextBox(ax_manual, 'Recorrido Manual:', initial='A,C,D')
